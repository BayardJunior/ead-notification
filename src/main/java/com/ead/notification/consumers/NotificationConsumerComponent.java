...
public class NotificationConsumerComponent {
    ...
    public NotificationConsumerComponent(NotificationService notificationService) {
    ...
    public void listenUserEvent(@Payload NotificationCommandDto commandDto) {
        NotificationModel model = new NotificationModel();
        BeanUtils.copyProperties(commandDto, model);
        model.setCreationDate(LocalDateTime.now(ZoneId.of("UTC")));
        model.setNotificationStatus(NotificationStatus.CREATED);
        notificationService.saveNotification(model);
    }
}
